...
class WebAuthController extends Controller
{
    ...
    public function authenticate(Request $request) { $credentials = $request->validate(['email'=>'required|email','password'=>'required|string']); if (!Auth::attempt($credentials, $request->boolean('remember'))) return back()->withErrors(['email'=>'Email atau password salah.'])->withInput(); $request->session()->regenerate(); ActivityLog::create(['user_id'=>auth()->id(),'action'=>'LOGIN','description'=>'Login ke platform']); if (auth()->user()->must_change_password) return redirect()->route('password.change')->with('info','Gunakan password sementara yang diberikan admin, lalu buat password baru.'); return redirect()->intended('/')->with('success', 'Selamat datang kembali, '.auth()->user()->name.'!'); }
    public function changePassword() { return view('auth.change-password'); }
    public function updatePassword(Request $request) { $data = $request->validate(['password'=>'required|string|min:8|confirmed']); $user = $request->user(); $user->update(['password'=>$data['password'],'must_change_password'=>false]); ActivityLog::create(['user_id'=>$user->id,'action'=>'PASSWORD_CHANGED','description'=>'Mengganti password sementara']); return redirect('/')->with('success','Password berhasil diubah. Selamat datang di Titip Kilat!'); }
